...
function FormListItemsResponse({ jsonForm, formRecord }) {
  ...
  const handleNavigateToResponses = () => {
    window.location.href = `/userDashboard/netSheets?formId=${formRecord.id}`;
  };
  ...
  const viewInGoogleSheets = async () => {
    if (
      window.confirm(
        "View in Google Sheets? This will save the data to your Google Drive."
      )
    ) {
      try {
      ...
      <h2 className="text-sm cursor-pointer">
        ...
        <div className="space-x-2">
          <Button
            ...
            onClick={downloadCSV}
            disabled={loading}
            className="bg-slate-white border border-black text-black hover:bg-transparent 
            hover:border-b-2 font-semi-bold"
          >
            {loading ? (
